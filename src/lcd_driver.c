...
#define LCD_RAM_SIZE 40

/* --- Internal state --- */
static const struct device *i2c_dev;
static uint8_t display_ram[20];
...
{
    memset(display_ram, 0x0F, sizeof(display_ram));
    return lcd_write_data(0, display_ram, sizeof(display_ram));
}
...
void lcd_test_walking_segments(void)
{
    ...
    LOG_INF("Starting walking segment test...");

    for (int s = 0; s < LCD_RAM_SIZE; s++) {
        for (int com = 0; com < 4; com++) {
            memset(ram, 0x00, sizeof(ram));
            ram[s] = (1 << com);

            int ret = lcd_write_data(0x00, ram, LCD_RAM_SIZE);
            if (ret < 0) {
                LOG_ERR("Write failed at s=%d com=%d: %d", s, com, ret);
                return;
            }

            LOG_INF("Lighting SEG%d COM%d", s, com);
            k_msleep(400);
        }
    }

    LOG_INF("Walking test done");
}